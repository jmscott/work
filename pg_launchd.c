/*
 *  Synopsis:
 *	launchd wrapper around postgresql server to do a fast halt with SIGINT
 *  Description:
 *	pg_launchd only exists because launchd can not send a SIGINT to do a
 *	fast shutdown.
 *	
 ...
 *	launchd.  The time to pause before Also, the pid file is forcibly removed.
 *  Usage:
 *	pg_launchd							\
 *		--SIGINT-pause 5					\
...
/*
 *  Write log message to standard error.
 */
static void
log(char *msg1)
{
...
log_wait_status(int status)
{
	...
	log2("postmaster process terminated", what);
}
...
catch_TERM(int sig)
{
	...
	log("sending fast shutdown signal INT to postmaster ...");
	if (kill(postmaster_pid, SIGINT) != 0) {
		exit_status = 2;
		ERROR2("kill(postmaster, INT) failed", strerror(errno));
		return;
	}
	log("postmaster got signal");

	int pause = SIGINT_pause;
	SIGINT_pause = 0;
	while (pause > 1) {
		log("pausing 1 second as postmaster stops gracefully");
		sleep(1);
		--pause;
		...
		if (pid < 0) {
			exit_status = 2;
			ERROR2("waitpid(postmaster TERM) failed",strerror(errno));
			return;
		}
